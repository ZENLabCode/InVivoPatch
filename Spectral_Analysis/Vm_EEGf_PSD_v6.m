...
%SAVE OUTPUT
savePath = 'X:\1 TIDIS Lab\Tiago\Campelo_Manuscript\Fig3_PFC_DAP5\RawData\';
saveFile = 'PFC_DAP5_Analysis_PSD_VmEEG.mat';
savecondition ='DAP5_';
...
tmp = dir(['X:\1 TIDIS Lab\Tiago\Ephys_PROCESSED\In_vivo\Awake_Sleep\PFC_Processed\Pharmacology\NMDAR_Blockers\*\*\*_vmStage.mat']);
tmp([tmp.isdir]) = [];
if isempty(tmp)
...
%                     mpxx_Vm(ind) = 0;
                    clear stagez
                    if sta == 1 && any (ismember (ID_traces.Quiet_Wake, bou))
                        stagez = 'Quiet_Wake';
                    elseif sta == 1 && any (ismember (ID_traces.Motion_Wake, bou))
                        stagez = 'Motion_Wake';
                    elseif sta == 2
                        stagez = stage;
                    elseif sta == 3
...
if true
    ...
    for sta = 1:size(stages,1)
        ...
        subplot (2,2,1)
        plot (x_t, 10*log(mea), 'Color', stages{sta,2});
        hold on
        curve1 = 10*log(mea + err);
        curve2 = 10*log(mea - err);
        x2 = [x_t, flip(x_t,2)];
        inBetween = [curve1; flip(curve2,1)];
        fill(x2, inBetween',stages{sta,2}','facealpha',.2,'EdgeColor','none');
        minY1 = min(mea-err);
        maxY1 = max(mea+err);
        title ('PSD EEG');
        xlabel ('Freq. (Hz)')
        ...
        Plot.yVm(:,sta) = ydata;
        groupColors = [groupColors; stages{sta,2}'];
