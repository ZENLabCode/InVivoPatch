...
startSearchPath = 'X:\1 TIDIS Lab\Tiago\Ephys_PROCESSED\In_vivo\Awake_Sleep\PFC_Processed\Pharmacology\NMDAR_Blockers\**'
...
margin   = 1;
durLimit = 2*margin + 10; %[s]
bout_duration = 10*fsDAT; %[s]
...
for fil = 1:numel(filesVM)
    ...
    for sta = 1:size(Stages,1)
        ...
        %Divide data into bouts
        for c = 1:N
            ind = indSTA(c)+indMAR:indEND(c)-indMAR;
            data = dat(ind);
            startIndex = 1;
            window = 1;
            wd_number = floor (numel(data)/bout_duration); %for pre-allocating the data
            total_windows{c} = wd_number;
            window_VM = NaN (bout_duration, wd_number);
            window_index = {};

            %Divide data into sub-boutus with duration defined by bout_duration
            while startIndex + bout_duration - 1  <= length (data) 
                Vm_window_data  = data (startIndex : startIndex + bout_duration -1);
                index_window = ind (startIndex : startIndex + bout_duration -1);
                
                if numel (Vm_window_data) == bout_duration && all (~isnan(Vm_window_data))
                    window_VM(:,window) = Vm_window_data;
                    window_index{window} = index_window ([1,end]);
                end
                
                %Move to nex window of analysis
                startIndex = startIndex + bout_duration;
                window = window+1;
            end %while condition to crop data into 5 seconds windows
            
            for d = 1:wd_number
                VM = window_VM(:,d);
                if numel (VM) == bout_duration 
                    indexVM = window_index{d};
                    res.(label){col} = VM; 
                    res.info.indBouts.(label)(col,:) = indexVM;
                end
                col = col + 1;
            end %loop across window data struct and alocate to a res file
        end
        
        %stage after bouts
        res.(sprintf('stageAfter%s',label)) = hypnogram(indEND+1);
        res.(sprintf('nWindows%s',label)) = total_windows(:);
    end
    ...
    sFile = sprintf('%s_vmStage_10sec.mat',tmp{1});
