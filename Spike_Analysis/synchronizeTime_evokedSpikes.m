...
files = ['X:\1 TIDIS Lab\Tiago\Ephys_PROCESSED\In_vivo\Awake_Sleep\RC_Processed\20211119_TC_P046_Day1\Evoked_Spikes\OpenBCI-*.txt'];
...
for fil = 1:noFIL
    ...
    for k = 1:noABF
        ...
        if isnan(dt) %for new estimation
            ...
            %point 1
            [~,ind0] = min(abs(tT-tV(indPV))); %start point
